...
export const Moves: {[moveid: string]: MoveData} = {
	...
	gmaxsteelsurge: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				if (pokemon.hasItem('heavydutyboots') || pokemon.hasItem('phantompresence')) return;
	...
	stealthrock: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				if (pokemon.hasItem('heavydutyboots') || pokemon.hasItem('stealthrock')) return;
	...
	toxicspikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem('heavydutyboots') || pokemon.hasItem('phantompresence')) {
					return;
